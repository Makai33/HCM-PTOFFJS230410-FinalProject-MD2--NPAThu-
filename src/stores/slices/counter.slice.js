...
const findAllUsers = createAsyncThunk("findAllUsers", async () => {
  let res = await axios.get(process.env.REACT_APP_SERVER_JSON + "userList");
  return res.data;
});

const createNewUsers = createAsyncThunk("createNewUsers", async (newUser) => {
  //http://localhost:4000/users
  let res = await axios.post(
    process.env.REACT_APP_SERVER_JSON + "userList",
    newUser
  );
  return res.data;
});

const deleteUserById = createAsyncThunk("deleteUserByid", async (userId) => {
  //http://localhost:4000/users/1
  let res = await axios.delete(
    process.env.REACT_APP_SERVER_JSON + "userList/" + userId
  );
  return userId;
});

const updateUser = createAsyncThunk("updateUser", async (dataObj) => {
  console.log("dataObj dataObj", dataObj);
  //http://localhost:4000/users/1   , editData
  let res = await axios.put(
    process.env.REACT_APP_SERVER_JSON + "userList/" + dataObj.userId,
    dataObj.editData
  );
  return res.data;
});

const setStatusUser = createAsyncThunk("setStatusUser", async (dataObj) => {
  console.log("dataObj dataObj", dataObj);
  //http://localhost:4000/users/1   , editData
